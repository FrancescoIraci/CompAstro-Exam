...
#SEVNPATH
#-------------------------------
SEVN="/Users/francescoiraci/Desktop/SEVN" #Complete path to the SEVN folder
...
#-------------------------------
if [ "$cmdargument" = "" ]; then
  LISTBIN="${SEVN}/resources/ICs/5_150Msun.txt" #Complete path to input file (list of binaries or single stars)
else
  LISTBIN=$cmdargument
fi
...
#OUTPUT
#-------------------------------
OUTPATH="5_150imf/Compactness/xiPatton/c_sevn_output" #Complete path to the output folder (the folder will be automatically created or cleaned if it already exists)
...
#METALLICITY
#-------------------------------
Z=0.002  #Stellar metallicity - [*list][number]. If list use the Z in the input file otherwise overwrite all the Zs.
...
SUPERNOVA="compact" #Prescription for the SN explosion mechanism - [list*] [rapid] [delayed] [compact]
